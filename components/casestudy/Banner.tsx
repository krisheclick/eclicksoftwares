...
const Banner = ({data, className = ''} : Props) => {
    ...
    return (
        <div className={`${Styles.cmsBanner} ${Styles[className] ?? ''}`} style={{background: `url('${process.env.NEXT_PUBLIC_MEDIA_URL}${poster}') center / cover`}}>
            <Container>
                <Row className='text-white align-items-center'>
                    <Col lg={6}>
                        <div className={Styles.bannerText}>
                            <h1 className={`title ${Styles.bannerTitle}`}>{title ? title : data?.proj_name}</h1>
                            {data?.proj_main_banner_description && (
